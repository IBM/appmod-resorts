...
class DefaultWeatherData {
  ...
  private static final Logger logger = Logger.getLogger(DefaultWeatherData.class.getName());
  ...
  public DefaultWeatherData(String city) {
    ...
    if (isSupportedCity) {
      ...
      logger.severe(
          "fail initializing DefaultWeatherData because the given city "
              + city
              + " is not supported");
      throw new UnsupportedOperationException(
          "City is invalid. It must be one of " + Arrays.toString(Constants.SUPPORTED_CITIES));
    }
  }
  ...
  public String getDefaultWeatherData() throws IOException {
    ...
    if (Constants.PARIS.equals(getCity())) {
      ...
      throw new UnsupportedOperationException(
          "The default weather information for the selected city: "
              + city
              + " is not provided.  Valid selections are: "
